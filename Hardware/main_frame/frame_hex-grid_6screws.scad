...
difference() {
  ...
  /***********smoth  rod to add extra support to the whole frame*****/
  translate([-smooth_rod_margin*5+88,X_smooth_rods_sep_projected+20,0]) {
    cylinder(r=M8_rod_diam/2,h=10*frame_thickness,center=true,$fn=40);
  ...
  translate([(frame_width-wall_thickness)/2,frame_height-frame_hole_height/2-bottom_thickness,0])
    difference(){
      ...
      difference(){
        ...
        union(){
          /***little hole between back rod support and hex grid****/
          translate([42.5,-34,0]) 
          cylinder(h=100,r=10,$fn=6, center=true);
          *cube([5,10,10]);
          /*****hex grid****/
          //translate([-43,-43,-12])
          translate([-40,-40,-12])
        ...
        //cylinder(h=100,r=20,$fn=6, center=true);
        //cylinder(h=101,r=15,$fn=6, center=true);
    }
    }
...
/*****Second support for the  frame @y   *****/
  difference(){
    union(){
    *mirror([0,0,1]){
     translate([frame_width/2,frame_height,frame_thickness-0.5-36])
      rotate([90,0,-90])
        bconcave_corner(cr=wall_thickness, cres=0, l=frame_width, th=0.5, ext_corner=true);
      }
      /**base modified***/
    translate([frame_width/2,frame_height-frame_thickness/4,30.5])
      cube([frame_width,frame_thickness/2,frame_thickness],center=true);
         
    translate([frame_width/2,frame_height-frame_thickness/4,20])
      cube([frame_width,frame_thickness/2,frame_thickness*2],center=true);
         }

    /***screws modified***/
    rotate([90,0,0]) translate([frame_width/2,frame_thickness/2,-frame_height]) {
    translate([-base_screw_distance,26,0])
      cylinder(r=base_screw_diameter/2,h=2*smooth_rod_screw_len,center=true,$fn=6);
    translate([base_screw_distance*0.8,26,0])
      cylinder(r=base_screw_diameter/2,h=2*smooth_rod_screw_len,center=true,$fn=6);
    translate([base_screw_distance*0.2,26,0])
      cylinder(r=base_screw_diameter/2,h=2*smooth_rod_screw_len,center=true,$fn=6);
    /********holes in the base****/
